...
sigmaPositionSensor = 100; % feet; 2 of these, one to measure x and one to measure y
...
% SigmaVDisturbance = 5*50; % std dev of disturbance on v
% SigmaPsiDisturbance = .8*50; % std dev of disturbance on psi
SigmaVDisturbance = 5; % std dev of disturbance on v
SigmaPsiDisturbance = .8; % std dev of disturbance on psi
G = [0 0; 0 0; 1 0; 0 1];
e = [SigmaVDisturbance^2 SigmaPsiDisturbance^2];
z = [sigmaPositionSensor^2 sigmaPositionSensor^2 sigmaVelocitySensor^2 sigmaHeadingSensor^2];
Q = diag(e);  
R = diag(z);  
...
while time < endtime
    ...
    % calculate the 4 estimates of derivative over the time interval from
    % t to t+dt
    ...
    % To calculate xest, we'll use a linear model (Ax+Bu) where A and X are
    % variable rather than just X

    x0 = xest(:, i);
    u0 = u;
    xdot0 = HW7derivative(x0, u, time);

    % update A with current values of x0
    A = [0 0 sind(x0(4)), x0(3) * cosd(x0(4)) * 6076/3600;
         0 0 cosd(x0(4)) x0(3) * sind(x0(4)) * 6076/3600;
         0 0 -Kv 0;
         -Kpsibyx 0 0 -Kpsi];
    xestdot1 = A * (xest(:, i) - x0) + B * (u - u0) + xdot0;
    xestdot2 = A * (xest(:, i) - x0 + ( xestdot1 * (dt/2) )) + B * (u - u0) + xdot0; 
    xestdot3 = A * (xest(:, i) - x0 + ( xestdot2 * (dt/2) )) + B * (u - u0) + xdot0;
    xestdot4 = A * (xest(:, i) - x0 + ( xestdot3 * dt )) + B * (u - u0)+ xdot0; 
...
%         display(yest);
        ...
        % calculate K
        K = P(:, :, i)*C'*inv(C*P(:,:,i)*C'+R);  
       
        % Add in the measurement's contribution to xestdot
        totalxestdot = totalxestdot + K*C*(x(:,i) - xest(:,i));  
       
        % correct our estimate of P
        P(:, :, i + 1) = (eye(size(K*C)) - K*C) *P(:,:,i+1);  
        mCntr = mCntr + 1;
    end
...
legend('Real Position', 'Measured position', 'KF estimate of position')
title('y by x position, feet')

subplot(4, 1, 3)
plot(time_arr, x(3, :), time_arr_s, y(3, :), time_arr, xest(3, :))
legend('Real Velocity', 'Measured Velocity', 'KF estimate of velocity')
title('Timeline of V, knots')

subplot(4, 1, 4)
plot(time_arr, x(4, :), time_arr_s, y(4, :), time_arr, xest(4, :), '-.')
legend('Real heading', 'Measured Heading', 'KF estimate of heading')
