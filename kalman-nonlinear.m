...
while time < endtime
    ...
    xestdot1 = HW7derivative(xest(:, i), u, time);
    xestdot2 = HW7derivative(x(:, i) + xestdot1 * dt / 2, u, time + dt / 2);;  
    xestdot3 = HW7derivative(x(:, i) + xestdot2 * dt / 2, u, time + dt / 2);  
    xestdot4 = HW7derivative(x(:, i) + xestdot3 * dt, u, time + dt);  
...
fprintf('Error in \x03C8: \t\x03bc = % 8.3f\t\x03C3 = % 8.3f\n', mean(x(4, :) - xest(4, :)), std(x(4, :) - xest(4, :)))
 display(std(yest(1, :) - y(1, :)))
% display(mean(x(1,:) - xest(1,:)))
% display(mean(x(1,:) - y(1,:)))
% display(mean(xest(1,:)))
%% Finally, plot!
subplot(4, 1, [1, 2])
