...
while (range(n) > RANGETHRESHOLD)
    ...
    sigma_i(n) = atan2(xdiff, ydiff) - interceptorheading(n); %3.1-11-16
    sigma_t(n) = -1 * (atan2(-ydiff, -xdiff) + targetheading(n) + pi/2);
    % CALCULATE the derivative of angle of line-of-sight using chain and quotient rules
    sigmadot_i(n) = calcSigmaDot( ...
        xdiff, ...
        ydiff, ...
    ...
    sigmadot_t(n) = calcSigmaDot( ...
        -ydiff, ...
        -xdiff, ...
        [interceptorvelocity(n), targetvelocity(n)], ...
        [interceptorheading(n), targetheading(n)], ...
        true ...
        ); %3.2-11-19  
    % here's where you need to figure out the interceptor's side acceleration to command for an intercept
    if MODEL_TYPE == 0
        interceptorsideacceleration(n) = -3 * interceptorvelocity(n) * sigmadot_i(n); %3.2-11-19
        targetsideacceleration(n) = 3 * targetvelocity(n) * sigmadot_t(n); %3.2-11-19

    elseif MODEL_TYPE == 1

       interceptorsideacceleration(n) = -1 * interceptorvelocity(n) * (4 * sigmadot_i(n) + (2 * sigma_i(n))/(range(n)/ interceptorvelocity(n)));
       targetsideacceleration(n) = 1 * targetvelocity(n) * (4 * sigmadot_t(n) + (2 * sigma_i(n))/(range(n)/ -interceptorvelocity(n)))
    end
...
%  UPDATE the TARGET velocity and heading based on its dynamics
    ...
    targetvelocity(n) = targetvelocity(n - 1);
    % HEADING is just a constant turn
%     targetheading(n) = targetheading(n - 1) + TARGETHEADINGCHANGE;
    targetheading(n) = targetheading(n - 1) - atan2((targetsideacceleration(n - 1) * timestep), targetvelocity(n - 1))

    % UNWRAP the heading of the TARGET
    if (targetheading(n) > 2 * pi) targetheading(n) = targetheading(n) - 2 * pi; end
%     if (interceptorheading(n) < targetheading(n) + deg2rad(10)) targetheading(n) = targetheading(n) + deg2rad(1); end
%     if (interceptorheading(n) > targetheading(n) - deg2rad(10)) targetheading(n) = targetheading(n) - deg2rad(1); end
%         && interceptorheading(n - 1) > targetheading(n - 1) + deg2rad(10)
%         sign = targetheading(n - 1) / (-1 * targetheading(n - 1));
%         targetheading(n) = targetheading(n - 1) - atan2((targetsideacceleration(n - 1) * timestep), targetvelocity(n - 1)) + sign * deg2rad(1)
...
subplot(3, 1, 2)
plot(time, targetheading * 180 / pi, time, interceptorheading * 180 / pi, time(1:n - 1), sigma_i * 180 / pi)
...
function sigmaDot = calcSigmaDot(dx, dy, V, SI, t)
    arguments
        ...
        t % boolean describing if we are seeking sigmadot of target
    end
    
    dxdt = V(1) * sin(SI(1)) - V(2) * sin(SI(2));
    dydt = V(1) * cos(SI(1)) - V(2) * cos(SI(2));
    if t
        sigmaDot = (1 / (1 + (dx/dy)^2)) * ((dy * dydt - dx * dxdt) / dy^2);
    else
